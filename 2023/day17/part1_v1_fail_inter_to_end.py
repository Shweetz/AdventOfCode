...
def move(beam, direction):
	...
	beam.path.append((beam.i, beam.j, beam.heat, beam.direction))
...
def beam_advance(beam):
	...
	if beam.i == len(lines) - 1 and beam.j == len(lines[0]) - 1:
		# reached end
		for step in beam.path:
			key = (step[0], step[1], step[3]) # i, j, direction
			if key in beams_ie:
				if beam.heat >= beams_ie[key]:
					# worse than another path to this tile/direction
					# (should not happen)
					continue
					
			# heat from here to end = total heat - start to here
			beams_ie[key] = beam.heat - step[2]
		print([(i, j) for (i, j, _, _) in beam.path])
		return beam.heat
	...
	if beam.heat > best_heat - fin_heat:
		# worse than another path to the end
		return False

	key = (i, j, direction)
	if key in beams_si:
		if beam.heat >= beams_si[key]:
			# worse than another path to this tile/direction
			return
	beams_si[key] = beam.heat

	# if key in beams_ie:
	# 	if beam.heat + beams_ie[key] < best_heat:
	# 		# from that point, the end is calculated, add its value
	# 		print([(i, j) for (i, j, _, _) in beam.path])
	# 		return beam.heat + beams_ie[key]
...
# print(best_path_to)
beams_todo = []
beams_si = {} # fastest start to intermediate point
beams_ie = {} # fastest intermediate to end point
energized = []
fin_heat = int(lines[-1][-1])
